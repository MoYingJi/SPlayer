...
import { resolveResourcesPath } from "../../utils/file-loader";
...
const resolveAfpDir = (): string | null => {
  const dir = app.isPackaged
    ? resolveResourcesPath("afp")
    : path.join(app.getAppPath(), "resources", "afp");
  const files = ["afp.mjs", "afp.wasm.mjs"];
  return dir && files.every((file) => fs.existsSync(path.join(dir, file))) ? dir : null;
};

const getWorker = (): Worker => {
  if (worker) return worker;
  // 注意：在 electron-vite 中，所有的 rollupOptions.input 都会被扁平化到 out/main 目录下，
  // 且除非在该 input 中显式指定路径结构（如 "workers/xxx"），否则也会平铺，这里我们已经在配置中指定了路径
  const entry = new URL("./workers/recognition.worker.js", import.meta.url);
