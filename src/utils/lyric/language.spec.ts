...
import { applyLyricLanguages, type LyricLineWithLanguage } from "./language";
...
describe("applyLyricLanguages", () => {
  it("用假名上下文将纯汉字行识别为日语", () => {
    ...
    assert.deepEqual(
      lines.map((line: LyricLineWithLanguage) => line.language),
      ["ja", "ja"],
    );
  });
  ...
  it("通过比例推断双语混合歌曲的纯汉字行", () => {
    const lines = [...Array(3).fill(createLine("爱")), createLine("君が好き")];

    applyLyricLanguages(lines);

    assert.deepEqual(
      lines.map((line: LyricLineWithLanguage) => line.language),
      [...Array(3).fill("zh-CN"), "ja"],
    );
  });
